...
def make_topic_res(author, author_topic, is_self):
    ...
    for message in all_messages:
        ...
        if message.parent_message:
            ...
            #留言
            msg_list.append({'id':message.id,'content':message.content,'publisher':message.publisher.nickname,'publisher_avatar': str(message.publisher.avatar),'created_time': message.created_time.strftime('%Y-%m-%d %H:%M:%S'),'reply':[]})
